...
LR: eventueel de mogelijkheid geven om classificatie info te kiezen? Maar dan kom je wel in de problemen met de mapping. 
Bottleneck hier is dat je dus enige voorkennis nodig hebt van R om selectief informatie toe te voegen, of je guidance document moet sterk genoeg zijn. 
...
```{r}
taxon %<>% mutate(references = raw_source) 
```
...
```{r}
raw_data %<>% mutate(taxonID = raw_taxon_id) 
```
...
```{r}
taxon %<>% mutate(taxonRank = str_to_lower(raw_rank))
```
