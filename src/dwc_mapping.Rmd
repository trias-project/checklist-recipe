...
```{r}
parsed_names <- rgbif::parsenames(unique(input_data$input_scientific_name))
```
...
input_data %<>% mutate(input_taxon_id = paste(
  "my_dataset_shortname", # Update with your dataset shortname
  "taxon",
...
```{r}
taxon %<>% mutate(language = "my_language")
```

### license

```{r}
taxon %<>% mutate(license = "my_license")
```

### rightsHolder

```{r}
taxon %<>% mutate(rightsHolder = "my_rightsHolder")
```
...
```{r}
taxon %<>% mutate(datasetID = "my_datasetID")
```

### institutionCode

```{r}
taxon %<>% mutate(institutionCode = "my_institutionCode")
```

### datasetName

```{r}
taxon %<>% mutate(datasetName = "my_datasetName")
```
