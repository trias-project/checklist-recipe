...
Proportion of taxa over kingdoms and countries, with totals:

```{r}
input_data %>%
  tabyl(country_code, kingdom) %>%
  adorn_totals("row") %>%
  adorn_percentages("row") %>%
  adorn_pct_formatting(rounding = "half up", digits = 0)
```

Proportion of taxa over kingdoms and occurrence status, with totals:

```{r}
input_data %>%
  tabyl(occurrence_status, kingdom) %>%
  adorn_totals("row") %>%
  adorn_percentages("row") %>%
  adorn_pct_formatting(rounding = "half up", digits = 0)
```

Proportion of taxa over kingdoms and estblishment means, with totals:

```{r}
input_data %>%
  tabyl(establishment_means, kingdom) %>%
  adorn_totals("row") %>%
  adorn_percentages("row") %>%
  adorn_pct_formatting(rounding = "half up", digits = 0)
```
