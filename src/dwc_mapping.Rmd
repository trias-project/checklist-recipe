...
input_data %<>% mutate(input_taxon_id = paste(
  "my_dataset_shortname", # Update with your dataset shortname, e.g. "alien-fishes-checklist"
  "taxon",
...
```{r}
taxon %<>% mutate(accessRights = "my_accessRights")
taxon %<>% mutate(rightsHolder = "my_rightsHolder") # e.g. "INBO"
```

### datasetID

```{r}
taxon %<>% mutate(datasetID = "my_datasetID") # e.g. "https://doi.org/10.15468/xvuzfh"
```

### institutionCode

```{r}
taxon %<>% mutate(institutionCode = "my_institutionCode") # e.g. "INBO"
```

### datasetName

```{r}
taxon %<>% mutate(datasetName = "my_datasetName") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
